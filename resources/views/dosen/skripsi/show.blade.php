<x-app-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            {{-- Approval Panel: Tampil jika dosen belum approve/reject penugasan --}}
            @php
                $isPembimbing1 = $skripsi->pembimbing1_id === $dosen->id;
                $isPembimbing2 = $skripsi->pembimbing2_id === $dosen->id;
                $approved = $isPembimbing1 ? $skripsi->pembimbing1_approved : $skripsi->pembimbing2_approved;
                $catatan = $isPembimbing1 ? $skripsi->pembimbing1_catatan : $skripsi->pembimbing2_catatan;
            @endphp
            @if($skripsi->pembimbing1_id || $skripsi->pembimbing2_id)
            <div class="card-saas p-6">
                <h3 class="font-semibold text-siakad-dark mb-4">Status Penerimaan Pembimbingan</h3>
                ...
                @if($approved === true)
                    <div class="flex items-center gap-3 p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                        ...
                        <div>
                            <p class="font-medium text-emerald-800">Anda telah menyetujui penugasan ini</p>
                            <p class="text-sm text-emerald-600">Mahasiswa: {{ $skripsi->mahasiswa->user->name }}</p>
                        </div>
                    </div>
                @elseif($approved === false)
                    <div class="p-4 bg-red-50 rounded-lg border border-red-200">
                        ...
                        @if($catatan)
                        <p class="text-sm text-red-700"><strong>Alasan:</strong> {{ $catatan }}</p>
                        @endif
                    </div>
                @else
                    {{-- Belum ada keputusan --}}
                    <div class="p-4 bg-amber-50 rounded-lg border border-amber-200 mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-sm font-medium text-amber-800">Anda ditugaskan sebagai pembimbing. Harap berikan keputusan Anda.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                        <form action="{{ route('dosen.skripsi.approve', $skripsi) }}" method="POST">
                            @csrf
                        ...
                        <button onclick="document.getElementById('modal-reject-skripsi').classList.remove('hidden')"
                    ...
                    {{-- Reject Modal --}}
                    <div id="modal-reject-skripsi" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
                            <h4 class="font-bold text-siakad-dark mb-2">Tolak Penugasan Pembimbingan</h4>
                            <p class="text-sm text-siakad-secondary mb-4">Berikan alasan penolakan. Admin akan diberitahu dan akan menentukan pengganti.</p>
                            <form action="{{ route('dosen.skripsi.reject', $skripsi) }}" method="POST">
                                @csrf
                                ...
                                <div class="flex gap-3 justify-end">
                                    <button type="button" onclick="document.getElementById('modal-reject-skripsi').classList.add('hidden')"
                ...
                <div class="p-5 border-b border-siakad-light/50">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <a href="{{ route('bimbingan.download', $bimbingan) }}" target="_blank" class="inline-flex items-center gap-1 text-xs text-siakad-primary mt-2">
