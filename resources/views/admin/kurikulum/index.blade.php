...
<div class="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
    <div>
        <p class="text-sm text-siakad-secondary dark:text-gray-400 hidden md:block">
            Kelola data kurikulum berdasarkan program studi
        </p>
    </div>
            ...
            <table class="hidden md:table w-full table-saas">
                <thead>
                    <tr class="bg-siakad-light/30 dark:bg-gray-900">
                        ...
                        <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary dark:text-gray-400 uppercase tracking-wider">Nama Kurikulum</th>
                        <th class="text-left py-3 px-5 text-xs font-semibold text-siakad-secondary dark:text-gray-400 uppercase tracking-wider">Tahun Mulai</th>
                ...
                <tbody>
                    @php $kurikulums = $kurikulum->where('prodi_id', $p->id); @endphp
                    @forelse($kurikulums as $idx => $k)
                    <tr class="border-b border-siakad-light/50 dark:border-gray-700/50 hover:bg-siakad-light/10 dark:hover:bg-gray-700/30 transition">
                        ...
                        <td class="py-4 px-5 text-sm font-medium text-siakad-dark dark:text-white">{{ $k->nama }}</td>
                        <td class="py-4 px-5 text-sm text-siakad-secondary dark:text-gray-400">{{ $k->tahun_mulai }}</td>
                        <td class="py-4 px-5">
                            @if($k->is_active)
                        ...
                        <td class="py-4 px-5 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <button onclick="editKurikulum({{ $k->id }}, '{{ $k->nama }}', '{{ $k->tahun_mulai }}', {{ $k->is_active ? 'true' : 'false' }}, {{ $p->id }})" class="p-2 text-siakad-secondary dark:text-gray-400 hover:text-siakad-primary dark:hover:text-blue-400 hover:bg-siakad-primary/10 dark:hover:bg-gray-700 rounded-lg transition">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>
                                <form action="{{ route('admin.kurikulum.destroy', $k) }}" method="POST" onsubmit="return confirm('Hapus kurikulum ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="py-8 text-center text-siakad-secondary dark:text-gray-400 text-sm">
                            Belum ada kurikulum di program studi ini
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div id="createModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md animate-fade-in">
            <div class="px-6 py-4 border-b border-siakad-light dark:border-gray-700">
                <h3 class="text-lg font-semibold text-siakad-dark dark:text-white">Tambah Kurikulum</h3>
            </div>
            <form action="{{ route('admin.kurikulum.store') }}" method="POST">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-siakad-dark dark:text-gray-300 mb-2">Nama Kurikulum</label>
                        <input type="text" name="nama" class="input-saas w-full px-4 py-2.5 text-sm dark:bg-gray-900 dark:border-gray-700 dark:text-white" placeholder="Contoh: Kurikulum MBKM 2023" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-siakad-dark dark:text-gray-300 mb-2">Tahun Mulai</label>
                        <input type="number" name="tahun_mulai" class="input-saas w-full px-4 py-2.5 text-sm dark:bg-gray-900 dark:border-gray-700 dark:text-white" placeholder="Contoh: 2023" required>
                    </div>
                    <div>
    ...
    <div id="editModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl w-full max-w-md animate-fade-in">
            <div class="px-6 py-4 border-b border-siakad-light dark:border-gray-700">
                <h3 class="text-lg font-semibold text-siakad-dark dark:text-white">Edit Kurikulum</h3>
            </div>
            <form id="editForm" method="POST">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-siakad-dark dark:text-gray-300 mb-2">Tahun Mulai</label>
                        <input type="number" name="tahun_mulai" id="editTahun" class="input-saas w-full px-4 py-2.5 text-sm dark:bg-gray-900 dark:border-gray-700 dark:text-white" required>
                    </div>
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
    ...
    <script>
        function editKurikulum(id, nama, tahun, active, prodiId) {
            document.getElementById('editForm').action = `/admin/master-data/kurikulum/${id}`;
