...
<body onload="window.print()">
    ...
    <div class="container">
        ...
        <!-- Course Table with Full Weight / Mutu Details -->
        <table class="course-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 10%;">Nilai Huruf (HM)</th>
                    <th style="width: 12%;">Nilai Angka (AM)</th>
                    <th style="width: 10%;">Bobot (N x K)</th>
            ...
            <tbody>
                ...
                @forelse($nilaiList as $nilai)
                    @php
                        $mk = $nilai->kelas->mataKuliah ?? null;
                        if (!$mk) continue;
                        $nilaiHuruf = $nilai->nilai_huruf ?? '-';
                        $isGraded = $nilaiHuruf !== '-';
                        $bobot = $isGraded ? ($bobotMap[$nilaiHuruf] ?? 0.0) : 0.0;
                        $mutu = $bobot * ($mk->sks ?? 0);
                    ...
                    <tr>
                        ...
                        <td class="center"><strong>{{ $nilaiHuruf }}</strong></td>
                        <td class="center">{{ $isGraded ? number_format($bobot, 1) : '-' }}</td>
                        <td class="center">{{ $isGraded ? number_format($mutu, 1) : '0' }}</td>
                        <td class="center">{{ $isGraded ? 'LULUS' : 'Belum Ada Nilai' }}</td>
                    </tr>
                @empty
        ...
        <div class="summary-container">
            <table class="summary-table">
                ...
                <tr><td class="label">Total Nilai Mutu</td><td class="separator">:</td><td>{{ number_format($totalMutu, 1) }}</td></tr>
                <tr><td class="label">Indeks Prestasi Semester (IPS)</td><td class="separator">:</td><td>{{ number_format($ips, 2, ',', '.') }}</td></tr>
                <tr><td class="label">Jumlah kredit maksimum semester depan</td><td class="separator">:</td><td>{{ $maxSks }} SKS</td></tr>
            </table>
        </div>
